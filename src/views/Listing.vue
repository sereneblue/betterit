  ...
  <div v-if="listingsLoaded" class="listings">
    ...
    <div v-else>
      <div
        ...
        class="listing__thread"
        v-for="thread in listings"
      ...
      >
        <div class="listing__list">
          <div class="listing__title">
            ...
            <span
              v-if="thread.domain.indexOf('self.') == -1"
              class="listing__domain"
              >&nbsp;({{ thread.domain }})</span
            >
            <span v-if="thread.over_18" class="nsfw-label">NSFW</span>
          </div>
          <div
            v-if="thread.post_hint || (thread.url && !thread.is_self)"
          ...
          <div class="listing__thread-info">
            <span class="listing__thread-info--score"
              >▲ {{ thread.score | abbr }}</span
            >
            <span class="listing__thread-info--author"
              >| by {{ thread.author }}</span
            >
            <router-link
              v-if="thread.subreddit != sub"
              :to="{
                name: 'subreddit',
                params: { subreddit: thread.subreddit, sort: 'hot' }
              }"
              class="listing__thread-info--sub"
              >{{ thread.subreddit_name_prefixed }}</router-link
            >
            <span class="listing__thread-info--time"
              >{{ thread.created_utc | since }} |</span
            >
            <router-link
              :to="{
                name: 'thread',
                params: { subreddit: thread.subreddit, id: thread.id }
              }"
              class="listing__thread-info--comments"
              >{{ thread.num_comments | abbr }} comments</router-link
            >
          </div>
        </div>
      </div>
...
export default {
  ...
  created: function() {
    this.$store.dispatch("getListings", this.$route.params);
  },
  computed: {
    error() {
